...
module.exports = function (s, conf) {
  ...
  if (so.strategy) {
    ...
    if (s.strategy.orderExecuted) {
      eventBus.on('orderExecuted', function(order, type) {
        s.strategy.orderExecuted(s, order, type, executeSignal)
      })
    }
  }
  ...
  function syncBalance (cb) {
    ...
    s.exchange.getBalance({currency: s.currency, asset: s.asset}, function (err, balance) { // @todo-serhii Тут додати async та нормально діставати дані без колбеків.
    ...
    syncBalance(function (err, { quote }) {
      ...
      if (signal === 'buy') {
        ...
        if (latest_high_buy && so.sell_cancel_pct != null && Math.abs(sell_loss) < so.sell_cancel_pct) {
          ...
          console.log('s.currency ', s.currency);
          console.log('sell_loss ', sell_loss);
          console.log('so.sell_cancel_pct ', so.sell_cancel_pct);
          console.log('latest_high_buy', latest_high_buy);
          return cb(null, null)
        }


        //let sell_loss = 19.8882 ? (0.00002467 - 19.8882) / 19.8882 * -100 : null
        ...
        if (s.sell_order && so.max_slippage_pct != null) {
          ...
          if (slippage > so.max_slippage_pct) {
            ...
            console.log('price ', price);
            console.log('s.currency ', s.currency);
            console.log('so.max_slippage_pct ', so.max_slippage_pct );
            console.log('slippage', slippage);
            console.log('s.sell_order ', s.sell_order);
            return cb(err)
          }
        }
  ...
  function executeOrder (order, trade_type) {
    ...
    let my_trade = {
      ...
      time: order.time,
      time_readable: new Date(order.time).toISOString(),
    ...
    eventBus.emit('orderExecuted', order, trade_type)
  }
